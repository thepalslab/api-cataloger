...
def scan(path, output, format, team, recursive):
    ...
    if path_obj.is_file():
        ...
        click.echo(
            f"✅ Found {len(catalog.entries)} API sources with {total_endpoints} total endpoints"
        )
    ...
    for entry_data in data.get("entries", []):
        for ep_data in entry_data.get("endpoints", []):
            ...
            if (
                ...
                or query_lower in method.lower()
                ...
                results.append(
                    {
                        ...
                        "source": entry_data.get("metadata", {}).get(
                            "source_file", "unknown"
                        ),
                    }
                )
...
def stats(catalog_file):
    ...
    total_endpoints = sum(
        len(entry.get("endpoints", [])) for entry in data.get("entries", [])
    )
