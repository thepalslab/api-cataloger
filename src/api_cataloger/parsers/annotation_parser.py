...
class AnnotationParser(BaseParser):
    ...
    def can_parse(self, file_path: Path) -> bool:
        ...
        try:
            ...
            return any(re.search(pattern, content, re.IGNORECASE) for pattern in annotation_indicators)
        except Exception:
            return False
    ...
    def _extract_annotated_endpoints(self, content: str, language: str) -> List[APIEndpoint]:
        """Extract endpoints from annotations."""
        endpoints = []
    ...
    def _find_function_documentation(self, content: str, annotation_pos: int) -> Dict[str, Any]:
        ...
        # Python docstring
        docstring_match = re.search(r'[\'\"]{3}(.*?)[\'\"]{3}\s*$', before_annotation, re.DOTALL)
        if docstring_match:
    ...
    def _extract_class_description(self, content: str) -> Optional[str]:
        ...
        class_doc = re.search(r'class\s+\w+.*?:\s*[\'\"]{3}(.*?)[\'\"]{3}', content, re.DOTALL)
        if class_doc:
