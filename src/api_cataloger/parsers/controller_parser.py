...
class ControllerParser(BaseParser):
    ...
    PATTERNS = {
        ...
        'spring': [
            ...
            (r'@RequestMapping\(.*path\s*=\s*[\'"]([^\'"]+)[\'"].*method\s*=\s*RequestMethod\.(\w+)', 'java'),
        ],
        # Django / Python
        'django': [
            (r'path\([\'"]([^\'"]+)[\'"]', 'py'),
    ...
    def _extract_endpoints(self, content: str, file_path: Path, framework: str) -> List[APIEndpoint]:
    ...
    def _create_endpoint_from_match(self, match, framework: str) -> Optional[APIEndpoint]:
